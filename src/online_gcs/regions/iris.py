...
class IRISRegionBuilder:
    ...
    def build_regions(
        self, num_seeds: int, collision_samples: int = 3, rng: Optional[np.random.Generator] = None
    ) -> float:
        if rng is None:
    ...
    def estimate_coverage(
        self, num_samples: int = 200, rng: Optional[np.random.Generator] = None
    ) -> float:
        if not self.regions:
            return 0.0
    ...
    def _solve_ik(self, goal_pos: np.ndarray, q_initial: np.ndarray) -> Optional[np.ndarray]:
        ...
        ik.AddPositionConstraint(gripper_frame, p_BQ, self.plant.world_frame(), goal_pos, goal_pos)
    ...
    def build_regions_from_seeds(
        self,
        seed_configs: List[np.ndarray],
        collision_samples: int = 3,
        existing_regions: Optional[List[HPolyhedron]] = None,
    ) -> float:
        """
        ...
        print(
            f"  Built {len(self.regions)} regions from {len(seed_configs)} seeds "
            f"(skipped {skipped}) in {self.build_time:.1f}s",
            flush=True,
        )
        return self.build_time
