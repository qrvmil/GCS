...
class RRTStarPlanner:
    ...
    def is_edge_collision_free(self, q1: np.ndarray, q2: np.ndarray, num_checks: int = 15) -> bool:
    ...
    def _try_connect_to_goal(
        self, nodes: List[RRTNode], q_goal: np.ndarray, goal_tolerance: float
    ) -> Optional[int]:
        best_idx = None
        best_cost = float("inf")
    ...
    def _find_near_nodes(self, nodes: List[RRTNode], q: np.ndarray, radius: float) -> List[int]:
        near_indices = []
        for i, node in enumerate(nodes):
    ...
    def _compute_cost(self, nodes: List[RRTNode], parent_idx: int, q_new: np.ndarray) -> float:
    ...
    def plan(
        self, q_start: np.ndarray, q_goal: np.ndarray, goal_tolerance: float = 0.15
        ...
        for iteration in range(self.max_iterations):
            ...
            if dist_to_goal <= goal_tolerance:
                if self.is_edge_collision_free(q_new, q_goal):
                    ...
                    if goal_cost < best_goal_cost:
                        if goal_idx is None:
                            goal_node = RRTNode(
                                config=q_goal.copy(), parent_idx=new_idx, cost=goal_cost
                            )
                            goal_idx = len(nodes)
            ...
            if goal_idx is None and (iteration + 1) % 500 == 0:
                ...
                if connect_idx is not None:
                    ...
                    goal_node = RRTNode(
                        config=q_goal.copy(), parent_idx=connect_idx, cost=goal_cost
                    )
                    goal_idx = len(nodes)
                    nodes.append(goal_node)
                    best_goal_cost = goal_cost
            ...
            if (iteration + 1) % 2000 == 0:
                ...
                print(
                    f"---[RRT*] Iteration {iteration + 1}/{self.max_iterations}, "
                    f"nodes={len(nodes)}, status={status}",
                    flush=True,
                )
    ...
    def plan_bidirectional(
        ...
        for iteration in range(self.max_iterations):
            ...
            if self.is_collision_free(q_new_a) and self.is_edge_collision_free(
                tree_a[nearest_a].config, q_new_a
            ):
                ...
                if dist <= goal_tolerance and self.is_edge_collision_free(
                    tree_b[nearest_b].config, q_new_a
                ):
                    connection = (idx_a, nearest_b)
                    break

                q_new_b = self._steer(tree_b[nearest_b].config, q_new_a)
                if self.is_collision_free(q_new_b) and self.is_edge_collision_free(
                    tree_b[nearest_b].config, q_new_b
                ):
                    cost_b = tree_b[nearest_b].cost + np.linalg.norm(
                        q_new_b - tree_b[nearest_b].config
                    )
                    tree_b.append(RRTNode(config=q_new_b.copy(), parent_idx=nearest_b, cost=cost_b))
                    idx_b = len(tree_b) - 1

                    dist = np.linalg.norm(q_new_b - q_new_a)
                    if dist <= goal_tolerance and self.is_edge_collision_free(q_new_a, q_new_b):
            ...
            if (iteration + 1) % 2000 == 0:
                print(
                    f"  [BiRRT] Iteration {iteration + 1}/{self.max_iterations}, "
                    f"tree_a={len(tree_a)}, tree_b={len(tree_b)}",
                    flush=True,
                )
...
def extract_keypoints(
    path: List[np.ndarray], max_points: int = 10, epsilon: float = 0.15, min_distance: float = 0.1
) -> List[np.ndarray]:
    if len(path) <= 2:
...
def _enforce_min_distance(path: List[np.ndarray], min_distance: float) -> List[np.ndarray]:
    if len(path) <= 2:
        return path
...
def extract_keypoints_uniform(path: List[np.ndarray], num_points: int = 10) -> List[np.ndarray]:
