...
def solve_ik(plant, plant_context, goal_pos, gripper_frame, q_initial) -> Optional[np.ndarray]:
    ...
    prog.AddConstraint(
        MinimumDistanceLowerBoundConstraint(
            plant=plant, bound=0.01, plant_context=plant_context, influence_distance_offset=0.01
        ),
        q,
    )

    ik.AddPositionConstraint(
        gripper_frame, np.array([0.0, 0.1, 0.0]), plant.world_frame(), goal_pos, goal_pos
    )
...
class GCSExperiment:
    ...
    def __init__(
        self,
        regions_dir: str = "artifacts/gcs-path/regions",
        output_dir: str = "artifacts/gcs-shelf",
    ...
    def _find_distant_pair(
        self, configs: List[np.ndarray], regions: List[HPolyhedron], min_dist: float = 2.0
    ) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """Find two configs in regions that are far apart."""
        # Filter configs that are in regions
        covered = [(q, i) for q in configs for i, r in enumerate(regions) if r.PointInSet(q)]
    ...
    def run_distant_experiments(
        self, scene_type: SceneType, num_seeds: int, regions: List[HPolyhedron]
    ) -> List[PathResult]:
        ...
        for trial in range(self.num_distant_trials):
            ...
            print(
                f"    [distant] trial {trial + 1}: {status} len={result.path_length:.2f}",
                flush=True,
            )
            results.append(result)
    ...
    def run_all(self):
        ...
        for scene in self.scenes:
            for num_seeds in self.seed_counts:
                ...
                if not regions:
                    print("  No regions found!", flush=True)
                    continue
                print(f"  Loaded {len(regions)} regions", flush=True)

                # Distant experiments
                print("  Running DISTANT experiments...", flush=True)
                ...
                # Shelf experiments
                print("  Running SHELF experiments...", flush=True)
...
def main():
    ...
    parser.add_argument(
        "--regions",
        type=str,
        default="artifacts/gcs-path/regions",
        help="Directory with pre-built IRIS regions",
    )
    parser.add_argument(
        "--output",
        type=str,
        default="artifacts/gcs-shelf",
        help="Output directory (default: artifacts/gcs-shelf)",
    )
    parser.add_argument(
        "--distant-trials", type=int, default=5, help="Number of distant path trials per config"
    )
