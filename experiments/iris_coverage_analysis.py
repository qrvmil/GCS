...
class IRISExperiment:
    ...
    def __init__(self, seed_counts: List[int], num_trials: int = 2,
                 random_seed_base: int = 42, coverage_samples: int = 200,
                 output_dir: str = "iris_results"):
    ...
    def run_single(self, scene_type: SceneType, num_seeds: int, trial: int) -> ExperimentResult:
        ...
        print(f"  Building scene and IRIS regions...", flush=True)
        ...
        # Save regions
        os.makedirs(f"{self.output_dir}/regions", exist_ok=True)
        builder.save_regions(f"{self.output_dir}/regions/{scene_type.name}_{num_seeds}_{trial+1}.yaml")
    ...
    def get_statistics(self) -> pd.DataFrame:
        ...
        stats.columns = ['scene', 'num_seeds', 'avg_regions',
                         'isolated_mean', 'isolated_std',
                         'isolation_rate_mean', 'isolation_rate_std',
    ...
    def save_results(self):
        ...
        raw_df.to_csv(f"{self.output_dir}/raw_{timestamp}.csv", index=False)
        
        stats = self.get_statistics()
        stats.to_csv(f"{self.output_dir}/stats_{timestamp}.csv", index=False)
...
def main():
    ...
    parser.add_argument("--output", type=str, default="iris_results")
