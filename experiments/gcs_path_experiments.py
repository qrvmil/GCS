"""
GCS path length experiments using the installed ``online_gcs`` package.
...
class GCSPathExperiment:
    ...
    def __init__(
        self, output_dir: str = "artifacts/gcs-path", shelf_ratio: float = 0.3, num_trials: int = 3
    ):
    ...
    def _find_distant_pair(
        self, configs: List[np.ndarray], min_dist: float = 2.0
    ...
    def _find_configs_in_regions(
        self, builder: IRISRegionBuilder, n: int, rng: np.random.Generator
    ) -> List[np.ndarray]:
    ...
    def run_single_scene(
        self, scene_type: SceneType, num_seeds: int, trial: int
        ...
        builder.build_regions_mixed(num_seeds=num_seeds, shelf_ratio=self.shelf_ratio, rng=rng)
        ...
        if len(covered_configs) >= 2:
            ...
            if pair:
                q_start, q_goal = pair
                print(
                    f"  [DISTANT] Planning path (dist={np.linalg.norm(q_start - q_goal):.2f})...",
                    flush=True,
                )
        ...
        print("  [SHELF] Finding shelf positions...", flush=True)
        shelf_positions = builder._get_shelf_positions()

        shelf_result = PathResult(
            scene=scene_type.name,
            num_seeds=num_seeds,
            experiment_type="shelf",
            trial=trial,
            success=False,
            path_length=0,
            solve_time=0,
        ...
        if len(shelf_positions) >= 2:
            ...
            if q_start is not None:
                ...
                if q_goal is not None:
                    ...
                    print(
                        f"  [SHELF] IK success. In regions: start={start_in}, goal={goal_in}",
                        flush=True,
                    )

                    print("  [SHELF] Planning path...", flush=True)
                    success = planner.solve_from_configs(
                        q_start, q_goal, build_missing_regions=True
                    ...
                    if start_in and goal_in:
                        print("  [SHELF] configs in regions", flush=True)
                    else:
                        print("  [SHELF] Configs not in regions", flush=True)
                else:
                    print("  [SHELF] IK failed for goal", flush=True)
            else:
                print("  [SHELF] IK failed for start", flush=True)
        ...
        for scene in self.scenes:
            for num_seeds in self.seed_counts:
                for trial in range(self.num_trials):
                    ...
                    print(
                        f"  Results: DISTANT {d_status} (len={distant_res.path_length:.2f}), "
    ...
    def get_statistics(self) -> pd.DataFrame:
        ...
        stats = (
            df.groupby(["scene", "num_seeds", "experiment_type"])
            .agg(
                {
            ...
            display = pd.DataFrame(
                {
                    ...
                    "Success": type_stats.apply(
                        lambda r: (
                            f"{int(r['success_count'])}/{int(r['total'])} ({r['success_rate'] * 100:.0f}%)"
                        ),
                        axis=1,
                    ),
                    "Path Length": type_stats.apply(
                        lambda r: (
                            f"{r['path_length_mean']:.2f}±{r['path_length_std']:.2f}"
                            if r["path_length_mean"] > 0
                            else "N/A"
                        ),
                        axis=1,
                    ),
                    "Solve Time": type_stats.apply(
                        lambda r: (
                            f"{r['solve_time_mean']:.3f}s" if r["solve_time_mean"] > 0 else "N/A"
                        ),
                        axis=1,
                    ),
                }
...
def main():
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="artifacts/gcs-path",
        help="Output directory (default: artifacts/gcs-path)",
    )
