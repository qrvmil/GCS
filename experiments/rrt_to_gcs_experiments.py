...
class RRTtoGCSExperiment:
    def __init__(self, 
                 output_dir: str = "rrt_gcs_results",
    ...
    def run_single_pair(self, scene_type: SceneType,
                        ...
                        rng_seed: int = 42) -> RRTtoGCSResult:
        ...
        print(f"---[Step 1] Running BiRRT...", flush=True)
        ...
        if path is None:
            print(f"---[BiRRT] Failed, trying RRT*...", flush=True)
            rrt_planner = RRTStarPlanner(
                scene_type=scene_type,
                max_iterations=self.max_iterations,
                step_size=self.step_size,
        ...
        if path is None:
            print(f"---[RRT*] Failed to find path!", flush=True)
            return result
        
        result.rrt_success = True
        result.rrt_path_length = rrt_planner.path_length
        result.rrt_path_points = len(path)
        print(f"---[RRT*] Success: length={result.rrt_path_length:.2f}, "
              f"points={len(path)}, time={result.rrt_solve_time:.2f}s", flush=True)
        
        print(f"---[Step 2] Extracting keypoints...", flush=True)
        ...
        print(f"---[Keypoints] Extracted {len(keypoints)} keypoints from {len(path)} path points", flush=True)
        
        print(f"---[Step 3] Building IRIS regions...", flush=True)
        ...
        print(f"---[IRIS] Built {result.num_regions_built} regions in {result.iris_build_time:.2f}s", flush=True)
        
        if result.num_regions_built == 0:
            print(f"---[IRIS] No regions built, skipping GCS", flush=True)
            return result
        
        region_file = self.regions_dir / f"{scene_type.name}_{start_idx}_{goal_idx}.yaml"
        iris_builder.save_regions(str(region_file))
        
        print(f"---[Step 4] Running GCS planner...", flush=True)
        ...
        if gcs_success:
            ...
            print(f"---[GCS] Success: length={result.gcs_path_length:.2f}, "
                  f"time={result.gcs_solve_time:.2f}s", flush=True)
        else:
            print(f"---[GCS] Failed to find path", flush=True)
    ...
    def run_scene(self, scene_type: SceneType):
        ...
        for pair_idx, (i, j) in enumerate(pairs):
            ...
            result = self.run_single_pair(
                scene_type, q_start, q_goal, i, j, rng_seed
            )
            self.results.append(result)
            
            if result.rrt_success and result.gcs_success:
                improvement = (result.rrt_path_length - result.gcs_path_length) / result.rrt_path_length * 100
                print(f"Summary: RRT*={result.rrt_path_length:.2f} -> GCS={result.gcs_path_length:.2f} "
                      f"({improvement:+.1f}%)")
            elif result.rrt_success:
                print(f"Summary: RRT* success ({result.rrt_path_length:.2f}), GCS failed")
            else:
                print(f"Summary: RRT* failed")
    ...
    def get_statistics(self) -> pd.DataFrame:
        ...
        stats.columns = [
            'scene',
            'rrt_success_count', 'total_pairs', 'rrt_success_rate',
            'gcs_success_count', 'gcs_success_rate',
            'rrt_path_length_mean', 'gcs_path_length_mean',
            'rrt_time_mean', 'gcs_time_mean', 'iris_time_mean',
            'keypoints_mean', 'regions_mean',
        ]
    ...
    def print_summary(self):
        ...
        for _, row in stats.iterrows():
            print(f"\n{row['scene']}:")
            print(f"  RRT* Success: {int(row['rrt_success_count'])}/{int(row['total_pairs'])} "
                  f"({row['rrt_success_rate']*100:.1f}%)")
            print(f"  GCS Success:  {int(row['gcs_success_count'])}/{int(row['total_pairs'])} "
            ...
            if row['rrt_success_count'] > 0:
                ...
                print(f"  Times (avg): RRT*={row['rrt_time_mean']:.1f}s, "
...
def main():
    ...
    parser.add_argument("--max-iterations", type=int, default=10000,
                        help="Max RRT* iterations (default: 10000)")
    parser.add_argument("--max-keypoints", type=int, default=10,
                        help="Max keypoints to extract (default: 10)")
    parser.add_argument("--step-size", type=float, default=0.3,
                        help="RRT* step size in radians (default: 0.3)")
    parser.add_argument("--epsilon", type=float, default=0.15,
                        help="Douglas-Peucker epsilon (default: 0.15)")
    parser.add_argument("--output", type=str, default="rrt_gcs_results",
                        help="Output directory (default: rrt_gcs_results)")
    parser.add_argument("--scene", type=str, default=None,
                        help="Run only specific scene (SINGLE_SHELF, TWO_SHELVES, TABLE_THREE_SHELVES)")
    parser.add_argument("--pair", type=str, default=None,
                        help="Run only specific pair, format: 'i,j' (e.g., '0,1')")
    ...
    if args.scene and args.pair:
        ...
        result = experiment.run_single_pair(
            scene_type, positions[i], positions[j], i, j
        )
